...
public class GeoFireAPI {
    ...
    static final String STORAGE_URL = "gs://aftarobot-2016.appspot.com/",
            TAG = GeoFireAPI.class.getSimpleName();
    ...
    public static void getLandmarks(double latitude, double longitude, double radius, StorageListener listener) {
        ...
        q.addGeoQueryEventListener(new GeoQueryEventListener() {
            ...
            public void onKeyEntered(String key, GeoLocation location) {
                ...
                DatabaseReference ref = db.getReference(DataAPI.AFTAROBOT_DB)
                        .child(DataAPI.CITIES).child(result[0])
                        .child(DataAPI.ROUTES).child(result[1])
                        .child(DataAPI.ROUTE_CITIES).child(result[2])
                        .child(DataAPI.LANDMARKS).child(result[3]);
